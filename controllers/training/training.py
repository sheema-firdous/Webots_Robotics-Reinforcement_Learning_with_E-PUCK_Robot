...
logging.basicConfig(
    ...
    #DEBUG messages are ignored unless we change the level to DEBUG.
...
gps.enable(timestep) # we can use gps.getValue to retrieve the robot's current position
...
# Initial positions
robot.step(timestep)
initial_position = robot_node.getField("translation").getSFVec3f() #getting 3D values of robot's position from the Supervisor
initial_rotation = robot_node.getField("rotation").getSFRotation() # current rotation,
# these variables were used incase i need to reset the robot's position or rotatio to original configuration, i can use these variable's values
...
while robot.step(timestep) != -1:
    ...
    state = (round(x, 2), round(y, 2)) # x and y were the robot's position, rounded them off to make it integer atleast
